...
class CategoriaAdapter(context:Context, private var items: List<Categoria>) : ArrayAdapter<Categoria>(context, 0, items) {
    ...
    private fun createViewFromResource(position: Int, convertView: View?, parent: ViewGroup, resource: Int): View {
        ...
        textView.text = getItem(position)?.nombreCategoria
        return view
    }
}